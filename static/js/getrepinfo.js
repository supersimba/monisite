...
function getTargetInfo(queue_id,trobj,i,trobj_plus) {
    $.ajax({
                ...
                success:function (callback) {
                    ...
                    {
                        trobj.children().eq(8).text(setSyncStatus(callback["sync_status"]));
                        trobj.children().eq(9).text(setSyncActive(callback["active"]));
                        trobj.children().eq(10).text(callback["add_time"]);
...
$(document).ready(function () {
    ...
    setInterval(displayTargetInfo,1000);
